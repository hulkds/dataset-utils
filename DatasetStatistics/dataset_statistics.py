...
def plot_labels_statistic(labels: np.ndarray, names=Dict[int, str], save_dir=Path("")):
    ...
    # seaborn correlogram
    sn.pairplot(
        boxes_data_frame, corner=True, diag_kind="auto", kind="hist", diag_kws={"bins": 50}, plot_kws={"pmax": 0.9}
    )
    ...
    # color histogram bars by class
    with contextlib.suppress(Exception):
        [
            y[2].patches[i].set_color([boxes_data_frame / 255 for boxes_data_frame in colors(i)])
